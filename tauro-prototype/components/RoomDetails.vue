<template>
  <div v-if="props.selectedRoomObject" class="room-details-card">
    ...
    <div class="room-details-text room-details-capacity">
      Capaciteit:
      <strong>{{ props.selectedRoomObject.capacity || 0 }} personen</strong>
    </div>
    <div
      class="room-details-text room-details-features"
      v-if="
        props.selectedRoomObject.features &&
        props.selectedRoomObject.features.length
      "
    >
      Features:
      <strong>{{ props.selectedRoomObject.features.join(", ") }}</strong>
    </div>
  </div>
</template>
