    ...
    <div class="content-grid">
      ...
      >
        <div class="info-alert">
          ...
          {{ props.cityName || "de geselecteerde stad" }}.
        </div>
      </div>
...
const bookedSlotsForSelectedDate = computed(() => {
  ...
  if (
    ...
    !dateStr ||
    !bookingsForSelectedRoom.value.length
  )
    return [];
  const bookingForDate = bookingsForSelectedRoom.value.find(
    (b) => b.date === dateStr
  );
...
const allTimesBookedForDate = computed(() => {
  ...
  return props.staticPossibleTimeSlots.every((slot) =>
    bookedSlotsForSelectedDate.value.includes(slot)
  );
});
...
const fullyBookedDatesForSelectedRoom = computed(() => {
  ...
  bookingsForSelectedRoom.value.forEach((booking) => {
    ...
    (booking.times || []).forEach((time) =>
      bookingsByDate[booking.date].add(time)
    );
  });

  for (const date in bookingsByDate) {
    if (
      props.staticPossibleTimeSlots.every((slot) =>
        bookingsByDate[date].has(slot)
      )
    ) {
...
const formattedSelectedDateShort = computed(() => {
  ...
  const date =
    localSelectedDate.value instanceof Date
      ? localSelectedDate.value
      : new Date(localSelectedDate.value);
  if (isNaN(date.getTime())) return "Ongeldige datum";
  return date.toLocaleDateString("nl-NL", { month: "long", day: "numeric" });
});

const isTimeSlotBooked = (time) =>
  bookedSlotsForSelectedDate.value.includes(time);
