<template>
  <div>
    <CitySelection @city-selected="handleCitySelected" />
...
const handleCitySelected = (selectedLocatieObject) => {
  console.log(
    "[pages/index.vue] Raw selectedLocatieObject from CitySelection:",
    JSON.parse(JSON.stringify(selectedLocatieObject))
  );

  if (
    selectedLocatieObject &&
    typeof selectedLocatieObject.value === "string" &&
    typeof selectedLocatieObject.name === "string"
  ) {
    currentCityFilterValue.value = selectedLocatieObject.value;
    currentCityDisplayName.value = selectedLocatieObject.name;
    console.log(
      `[pages/index.vue] currentCityFilterValue updated to: '${currentCityFilterValue.value}', currentCityDisplayName: '${currentCityDisplayName.value}'`
    );
    ...
    console.warn(
      "[pages/index.vue] Received selectedLocatieObject is not in the expected format (missing value or name):",
      selectedLocatieObject
    );
  }
};
