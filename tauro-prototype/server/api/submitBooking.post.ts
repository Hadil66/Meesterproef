...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    console.log(
      "[Server] Received booking payload. Room ID:",
      clientPayload?.acf?.booking_room_id,
      "Date:",
      clientPayload?.acf?.booking_date
    );
    ...
    if (
      !clientPayload?.acf?.booking_room_id ||
      !clientPayload?.acf?.booking_date ||
      !clientPayload?.acf?.booking_time_slots?.length
    ) {
      console.error(
        "[Server] Validation failed: Missing essential booking fields."
      );
      throw createError({
        statusCode: 400,
    ...
    const wordpressPostBody = {
      title:
        clientPayload.title ||
        `Reservering: ${clientPayload.acf.booking_room_id} op ${clientPayload.acf.booking_date}`,
    ...
    const wpResponse = await $fetch(fullWordPressBookingEndpoint, {
      ...
      console.error(
        "[Server] WordPress API Error. Status:",
        wpError.response?.status,
        "Data:",
        JSON.stringify(wpError.data, null, 2)
      );
      const errorMessage =
        wpError.data?.message ||
        wpError.message ||
        "Onbekende WordPress API fout";
      throw createError({
    ...
    if (wpResponse && (wpResponse as any).id) {
      ...
      console.error(
        "[Server] Unexpected WordPress API Response structure:",
        wpResponse
      );
      throw createError({
        statusCode: 502,
    ...
    console.error(
      "[Server] Error in submitBooking route:",
      error.message,
